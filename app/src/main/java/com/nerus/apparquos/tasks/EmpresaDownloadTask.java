...
public class EmpresaDownloadTask extends AsyncTask<String, Integer, Object> {
    public static final String TASK_NAME="EmpresaDownloadTask";
    private TasksCallBacks mCallBacks;
    private String mUrlApi;
    private String mTokenApp;
    private String mTokenUser;
    private Empresa mToReturn;

    public EmpresaDownloadTask(String urlApi, String tokenApp, String tokenUser ) {
        super();
        mUrlApi=urlApi;
        mTokenApp=tokenApp;
    ...
    public EmpresaDownloadTask setCallBacks(TasksCallBacks callBacks) {
                ...
                if (jsonResult.has("data") == true) {
                    try {
                        ...
                        for (Integer i = 0; i < jArray.length(); i++) {
                            ...
                            mToReturn = new Empresa(oneObject.getString("rfc")
                                    ,oneObject.getString("descripcion")
                                    ,oneObject.getString("fecha")
                                    ,oneObject.getString("direccion")
                            );


                            LogSNE.d("NERUS", "Object :" + i.toString() + ".-" + mToReturn.toString());
                        }

                        if (mToReturn==null){
                            mToReturn = Empresa.getHint();
                        }
                        return "OK";
        ...
        if (mCallBacks != null ) {
            ...
            if (result instanceof String) {
                ...
                if (cResult.equals("OK")) {
                    mCallBacks.onRequestSuccess(TASK_NAME, mToReturn);
                } else {
