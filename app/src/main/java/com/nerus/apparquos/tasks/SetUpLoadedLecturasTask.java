...
public class SetUpLoadedLecturasTask extends AsyncTask<List<Lectura>, Integer, Object> {
    public static final String TASK_NAME="SetUpLoadedLecturasTask";
    private TasksCallBacks mCallBacks;
    private LecturaDAO mAsyncObjectDAO;
    public SetUpLoadedLecturasTask(LecturaDAO objectDAO) {
        super();
        mAsyncObjectDAO = objectDAO;
    }

    public SetUpLoadedLecturasTask setCallBacks(TasksCallBacks callBacks) {
        mCallBacks = callBacks;
        return this;
    }

    @Override
    protected Object doInBackground(List<Lectura>... values) {
        List<Lectura> lecturas =  values[0];
        String cFecha = clsFecha.getFechaYMDhms();
        for (Lectura lec : lecturas) {
            mAsyncObjectDAO.update( cFecha , lec.getIdPadron());
        }
        return "OK";
    }
